...
- **`components/`**: `ScreenContainer`, `SectionCard`, `StudentIconTile` (memoized grid tile, damage-type colored rim), `FilterChip`, `TypePill` (damage/armor pills), `StatBadge` (label-over-value block), `Button`, `ImageViewer`, `LevelControl` (stepper with min/MAX chips and a `valuePrefix` prop — `'Lv. '` default, `'T'` for gear tiers), `StatsSection` (level/star/UE-driven stat grid — gold char stars plus blue weapon stars — plus a collapsible "Gear · Bond · Talent" area: 3 gear-tier steppers T0–T10, bond rank 1–100 with one extra row per playable alt (`hasAlts` = SchaleDB `FavorAlts`, each alt applies its own favor table at its own rank — note Shiroko*Terror is bond-separate from Shiroko despite the name), 3 talent steppers 0–25, bond-gear T2 toggle; `ueStars` is a controlled prop owned by `app/student/[id].tsx` so the Combat card's mood row can render the UE 3★+ terrain boost as base → boosted, e.g. S → SS), `SkillCard` + `SkillsSection` (EX/Basic/Enhanced/Sub with Bond Gear / UW 2★ variant toggles).
