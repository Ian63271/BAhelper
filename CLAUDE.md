...
- **Screens**:
  ...
  - `student/[id].tsx` — full profile: portrait hero with school-icon watermark, combat/lore/weapon/bond-gear sections, alt-family navigation, header toggles for favorite/owned.
...
- **Data model**: `types/students.ts` defines the `Students` interface — a superset schema modeled after SchaleDB's fields; most fields optional. Notes: `mood` = [Urban, Outdoor, Indoor] adaptation values 0–4 (D→S); `limited` = per-region status array (1=Limited, 2=Event, 3=Fes; other values mean permanent).
- **Student data pipeline (local-only, gitignored dev tooling)**: `data/scrapper.js` (fetches SchaleDB `students.min.json` → `data/students.json`), `data/imagescrapper.js` / `data/iconscrapper.js` (portraits/icons into `assets/images/students|icons`), `data/schooliconscrapper.js` (school icons into `assets/images/schools`, pulled from the SchaleDB GitHub repo since the SPA swallows direct image paths), `data/localizationscrapper.js` (regenerates `constants/clubLabels.ts` from SchaleDB's `localization.min.json` — student records carry internal club keys like `Kohshinjo68` that need this map for display names), and `data/imagemap.js` (regenerates `types/imageMap.ts`).
  - `types/imageMap.ts` (tracked) holds static `require(...)` maps: `studentPortraits`, `studentHalos`, `studentIcons` (keyed by student ID) and `schoolIcons` (keyed by school name). Metro needs static requires — never load images by dynamic path.
...
- **`components/`**: `ScreenContainer`, `SectionCard`, `StudentIconTile` (memoized grid tile, damage-type colored rim), `FilterChip`, `TypePill` (damage/armor pills), `StatBadge` (label-over-value block), `Button`, `ImageViewer`.
- **`assets/` is entirely gitignored** — a fresh clone must run the pipeline scripts (`scrapper.js`, `imagescrapper.js`, `iconscrapper.js`, `schooliconscrapper.js`, then `imagemap.js`; plus `localizationscrapper.js` after a data refresh) to populate images and regenerate labels.
